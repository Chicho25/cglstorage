...
*/
	...
	*/
	...
	function getMaxReceiptNo()

	{

		$ReceiptNo = mysql_fetch_array(MySQLQuery("SELECT MAX(IFNULL(receipt_no,0)) AS ReceiptNo FROM receipt"));

		if(isset($ReceiptNo['ReceiptNo']) && $ReceiptNo['ReceiptNo'] > 0)

			$nReceipt = $ReceiptNo['ReceiptNo'] + 1;
	...
	function convertNum($num) {
	 ...
	 $num = (int) $tempNum[0];    // make sure it's an integer
	 ...
	 $leftNum = convertTri($num, 0)." Dollars ";

	 if($tempNum[1] != 0)

	 $leftNum.= " and ".convertTri($tempNum[1], 0)." cents";
